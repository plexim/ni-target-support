...
#include str(sub(BASE_NAME)_plx.h)

#define _MAKE_CHECKSUM_VAR_NAME(b) b##_checksum
#define MAKE_CHECKSUM_VAR_NAME(b) _MAKE_CHECKSUM_VAR_NAME(b)
#define CHECKSUM_VAR_NAME MAKE_CHECKSUM_VAR_NAME(BASE_NAME)

extern const char * const CHECKSUM_VAR_NAME;
...
NI_Version NIVS_APIversion DataSection(".NIVS.APIVersion") = {NIMF_VER_MAJOR, NIMF_VER_MINOR, NIMF_VER_FIX, NIMF_VER_BUILD};
	...
	HANDLE CreateSemaphore(void* lpSemaphoreAttributes, int lInitialCount, int lMaximumCount, char* lpName)
	{
		...
		MacSemaphore_t *sem = malloc(sizeof(MacSemaphore_t));
		if(sem == 0)
		{
			return NULL;
		}
		...
		int result;
		result = pthread_mutex_init(&sem->mutex, NULL);
		if (result)
		{
			free(sem);
			return NULL;
	...
	void WaitForSingleObject(HANDLE hHandle, int dwMilliseconds)
	{
		MacSemaphore_t *sem = (MacSemaphore_t *)hHandle;
		int error;
	...
	void ReleaseSemaphore(HANDLE hSemaphore, int lReleaseCount, void* lpPreviousCount)
	{
		...
		MacSemaphore_t *sem = (MacSemaphore_t *)hSemaphore;
	...
	void CloseHandle(HANDLE hSemaphore)
	{
		MacSemaphore_t *sem = (MacSemaphore_t *)hSemaphore;
	    pthread_mutex_destroy(&sem->mutex);
...
DLL_EXPORT int32_t PLX_GetModelChecksum(char* checksum, int32_t* checksumlen)
{
	int32_t len = 0;
	char *ptr = (char *)CHECKSUM_VAR_NAME;
