...
extern "C" {
	...
	#define DataSection(t) __attribute__ ((section((t))))
		...
		#endif
	#endif
	#define DataSection(t) // __attribute__ ((section((t)))) <- must be ".NIVS, paramlist"  and not ".NIVS.paramlist"
#else
	/* Windows macros */
    #ifdef __cplusplus
...
/*========================================================================*
 * Function: PLX_GetModelChecksum
 *
 * Abstract:
 *	Get the model checksum.
 *
 * Output Parameters:
 *	checksum			: checksum of the model
 *	checksumlen		: the checksum string length. If a value of "-1" is specified, the minimum buffer size of "checksum" is returned
 *
 * Returns:
 *	NI_OK if no error
 *========================================================================*/
